    ...
    if menu == "🛒 Tienda Online":
        ...
        with tab_tienda:
            ...
            if zona == "Otro (Consultar por WA)": 
                st.warning("⚠️ El delivery automático es solo para Aeropuerto y Playa Rímac."); st.stop()
        ...
        with tab_carrito:
            ...
            if st.session_state.carrito:
                ...
                with st.container(border=True):
                    ...
                    if metodo == "Yape / Plin":
                        st.markdown('<div style="background:#00D1B2; color:white; padding:15px; border-radius:10px; text-align:center;"><b>Yape/Plin al: 963 142 733</b><br>Paula Ottiniano</div>', unsafe_allow_html=True)
                        cap_pago = st.file_uploader("Sube una captura", type=['png','jpg','jpeg'])
                    else:
                        vuelto_de = st.number_input("¿Con cuánto pagarás?", min_value=total)

                    st.info(f"💡 Vecino/a, vas a pedir **{len(st.session_state.carrito)} productos**. Total: **S/ {total:.2f}**.")

                    if st.button("🚀 CONFIRMAR Y ENVIAR PEDIDO", type="primary", use_container_width=True):
                        if not u_cel or not u_dir or len(u_cel) < 9:
                            st.error("Revisa tu celular y dirección.")
                        else:
                            ...
                            nuevo_pedido = pd.DataFrame([{
                                ...
                                "metodo_pago": metodo,
                                "monto_pagado": vuelto_de if metodo == "Efectivo" else total,
                                "captura_pago": p_path,
                                "estado": "Nuevo"
                            }])
